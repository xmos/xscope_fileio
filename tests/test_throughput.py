...
def run_throughput(test_size=10, timeout=30):
    ...
    pr.terminate()
    assert return_code == 0, f"ERROR: xscope_fileio process failed with exit code {return_code}"
...
def test_throughput_1(test_size):
    run_throughput(test_size, timeout=80)
...
if __name__ == "__main__":
    ...
    fn_run_throughput(64, args.adapter_id)
